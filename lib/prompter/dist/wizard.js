...
function markDepsHint(root2) {
  runBridgeSync(root2, ["mark-deps-hint"]);
}
...
function colorizeStatusLines(body) {
  return body.split("\n").map((line) => {
...
`
    ...
    process.stdout.write(
      `
  ${styleSuccess2(`\u2713 ${mf("", ctx, "banner.version_ok", `v${v}`)}`)}
`
    );
  }
  process.stdout.write("\n");
...
function buildMenuOptions(ctx) {
  ...
  const addOpt = (n2) => {
    ...
    const styledLabel = isRec ? `${iconStep()} ${styleSuccess2(badge)}  ${styleLabel(label)}` : n2 === 5 ? `${styleWarning("\u25CB")} ${styleLabel(label)}` : styleLabel(label);
    options.push({
      value: String(n2),
      label: styledLabel,
      hint: isRec ? styleHint(recommendedHint(ctx)) : void 0
    });
  };
...
function buildStyledStatusNote(ctx) {
  if (ctx.assess.systemReady) {
    const lines2 = [
      `${iconOk()} ${styleSuccess2(ctx.assess.hint)}`
    ];
    if (!ctx.runtime.depsHintShown) {
      lines2.push("");
      lines2.push(styleInfo(m("", ctx, "runtime.deps_cleanup").split("\n")[0] ?? ""));
      const depsBody = m("", ctx, "runtime.deps_cleanup").split("\n").slice(1).join("\n");
      if (depsBody.trim()) {
        lines2.push(colorizeStatusLines(depsBody));
      }
      lines2.push("");
      lines2.push(styleMuted(m("", ctx, "runtime.deps_cleanup_short")));
    }
    return lines2.join("\n");
  }
  const lines = [

...
function block({
  ...
  return t2 && r2.write(import_sisteransi.cursor.hide), e.once("keypress", n2), () => {
    e.off("keypress", n2), t2 && r2.write(import_sisteransi.cursor.show), e instanceof import_node_tty.ReadStream && e.isTTY && !R && e.setRawMode(false), s.terminal = false, s.close();
  };
}
...
var T$1 = class T extends V {
  ...
  #l(e, t2) {
    ...
    i2 || s ? (this.#e = findCursor(this.#e, i2 ? -1 : 1, this.filteredOptions), this.focusedValue = this.filteredOptions[this.#e]?.value, this.multiple || (this.selectedValues = [this.focusedValue]), this.isNavigating = true) : n2 ? this.value = m(this.multiple, this.selectedValues) : this.multiple ? this.focusedValue !== void 0 && (t2.name === "tab" || this.isNavigating && t2.name === "space") ? this.toggleSelected(this.focusedValue) : this.isNavigating = false : (this.focusedValue && (this.selectedValues = [this.focusedValue]), this.isNavigating = false);
  }
  deselectAll() {
    this.selectedValues = [];
  }
...
var n = class extends V {
  get userInputWithCursor() {
    ...
    return `${e}${(0, import_node_util.styleText)("inverse", s)}${r2.join("")}`;
  }
  get cursor() {
    return this._cursor;
  }
...
var limitOptions = ({
  ...
  const i2 = getColumns(c2) - O, I = getRows(c2), C2 = (0, import_node_util2.styleText)("dim", "..."), x = Math.max(I - r2, 0), m2 = Math.max(Math.min(h2, x), 5);
  let p = 0;
  l2 >= m2 - 3 && (p = Math.max(
...
var autocomplete = (t2) => new T$1({
  ...
  render() {
    const n2 = t2.withGuide ?? settings.withGuide, i2 = n2 ? [`${(0, import_node_util2.styleText)("gray", S_BAR)}`, `${symbol(this.state)}  ${t2.message}`] : [`${symbol(this.state)}  ${t2.message}`], l2 = this.userInput, o2 = this.options, m2 = t2.placeholder, p = l2 === "" && m2 !== void 0, $ = (r2, s) => {
...
`).map(c2), ""], n2 = dist_default2(e), r2 = Math.max(
    ...
    (m2) => `${(0, import_node_util2.styleText)("gray", S_BAR)}  ${m2}${" ".repeat(r2 - dist_default2(m2))}${(0, import_node_util2.styleText)("gray", S_BAR)}`
  ).join(`
...
var text = (t2) => new n({
  ...
  input: t2.input,
  render() {
    const i2 = t2?.withGuide ?? settings.withGuide, s = `${`${i2 ? `${(0, import_node_util2.styleText)("gray", S_BAR)}
` : ""}${symbol(this.state)}  `}${t2.message}
`, c2 = t2.placeholder ? (0, import_node_util2.styleText)("inverse", t2.placeholder[0]) + (0, import_node_util2.styleText)("dim", t2.placeholder.slice(1)) : (0, import_node_util2.styleText)(["inverse", "hidden"], "_"), o2 = this.userInput ? this.userInputWithCursor : c2, a2 = this.value ?? "";
    switch (this.state) {
      case "error": {
        const n2 = this.error ? `  ${(0, import_node_util2.styleText)("yellow", this.error)}` : "", r2 = i2 ? `${(0, import_node_util2.styleText)("yellow", S_BAR)}  ` : "", d = i2 ? (0, import_node_util2.styleText)("yellow", S_BAR_END) : "";
        return `${s.trim()}
${r2}${o2}
${d}${n2}
`;
      ...
      case "submit": {
        const n2 = a2 ? `  ${(0, import_node_util2.styleText)("dim", a2)}` : "", r2 = i2 ? (0, import_node_util2.styleText)("gray", S_BAR) : "";
        return `${s}${r2}${n2}`;
      }
      case "cancel": {
        const n2 = a2 ? `  ${(0, import_node_util2.styleText)(["strikethrough", "dim"], a2)}` : "", r2 = i2 ? (0, import_node_util2.styleText)("gray", S_BAR) : "";
        return `${s}${r2}${n2}${a2.trim() ? `
${r2}` : ""}`;
      }
      default: {
        const n2 = i2 ? `${(0, import_node_util2.styleText)("cyan", S_BAR)}  ` : "", r2 = i2 ? (0, import_node_util2.styleText)("cyan", S_BAR_END) : "";
        return `${s}${n2}${o2}
${r2}
`;
      }
...
function styleTitle(text2) {
  return `${theme.brand}${B}${text2}${R2}`;
}
function styleMessage(text2) {
  return `${theme.brand}${text2}${R2}`;
}
...
function readPayload() {
  ...
  if (fileIdx >= 0 && process.argv[fileIdx + 1]) {
    const raw = (0, import_node_fs.readFileSync)(process.argv[fileIdx + 1], "utf-8").trim();
    if (!raw) return {};
    return JSON.parse(raw);
  }
  if (process.argv.includes("--stdin")) {
    const raw = (0, import_node_fs.readFileSync)(0, "utf-8").trim();
    if (!raw) return {};
    return JSON.parse(raw);
  }
  return {};
...
function requireOptions(options, cmd) {
  if (options.length === 0) {
...
async function main() {
  ...
  switch (cmd) {
    ...
    case "outro":
      ...
      break;
    case "note":
      note(data.message ?? "", data.title ? styleTitle(data.title) : void 0);
      break;
    case "select": {
    ...
    case "spin": {
      ...
      const result = (0, import_node_child_process.spawnSync)(shellCmd, {
        shell: true,
        stdio: "inherit",
        env: process.env
      });
